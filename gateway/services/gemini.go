...
// GetGeminiHealth checks if Gemini service is healthy
func GetGeminiHealth() error {
	initGeminiClient()
	...
	reqBody := GeminiRequest{
		...
		GenerationConfig: struct {
			...
			MaxOutputTokens: 5, // Minimal response
		},
	}

	jsonData, err := json.Marshal(reqBody)
	if err != nil {
		return fmt.Errorf("error marshaling health check request: %v", err)
	}
...
// WarmupGeminiModel preloads a model to reduce first-request latency
func WarmupGeminiModel(model string) error {
	initGeminiClient()
		...
		Contents: []GeminiContent{
			{
				Parts: []struct {
					...
					{Text: "Hello"}, // Simple warmup prompt
				},
			},
		},
		GenerationConfig: struct {
			...
			MaxOutputTokens: 1, // Minimal response
		},
	}
	...
	if resp.StatusCode == http.StatusOK {
		log := logger.GetLogger("gemini.warmup")
		log.InfoWithFields("Gemini model warmed up successfully", map[string]interface{}{
			"model": modelName,
		})
	}
